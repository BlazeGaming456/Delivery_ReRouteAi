...
export default function MapComponent () {
  ...
  const displayAStarPath = (map, graph, path, wareHouseLocations) => {
    ...
    const origin = {
      lat: wareHouseLocations[path[0]].coordinates[0],
      lng: wareHouseLocations[path[0]].coordinates[1]
    }
    ...
    const waypoints = path.slice(1, -1).map(id => ({
      location: {
    ...
    directionsService.route(
      {
        ...
        optimizeWaypoints: false // true if you want shortest among permutations
      },
      (result, status) => {
        if (status === 'OK') {
  ...
  const handleClickPath = (map, newMarkers, setMarkers) => {
  ...
  const initMap = () => {
    ...
    const newMarkers = []
    handleClickPath(map, newMarkers, setMarkers)
